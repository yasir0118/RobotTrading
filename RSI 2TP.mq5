...
input double InpTP2Percentage   = 61.8;    // Persentase posisi profit untuk close all (%)
...
int OnInit()
{
   ...
   Print("TP2 Profit Percentage: ", InpTP2Percentage, "%");
...
void OnTick()
{
   ...
   {
      CheckTP2_Buy(buy_count);
   }
   
   // ========== CEK TP2 untuk SELL ==========
   if(sell_count > InpTP2MinPositions)
   {
      CheckTP2_Sell(sell_count);
   }
...
//+------------------------------------------------------------------+
//| Fungsi untuk cek TP2 pada posisi BUY                            |
//+------------------------------------------------------------------+
void CheckTP2_Buy(int total_buy)
{
   int profit_count = 0;
   double current_bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   
   // Hitung berapa posisi BUY yang sedang profit
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      if(ticket > 0)
      {
            ...
            PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY)
         {
            ...
            // BUY profit jika current_bid > open_price
            if(current_bid > open_price)
            {
               profit_count++;
            }
         }
   ...
   double profit_percentage = (double)profit_count / (double)total_buy * 100.0;
   
   // Jika persentase profit >= threshold, close all BUY
   if(profit_percentage >= InpTP2Percentage)
   {
      Print("TP2 BUY TRIGGERED! Profit: ", profit_count, "/", total_buy, " (", 
            DoubleToString(profit_percentage, 2), "%) - Closing all BUY positions...");
...
//+------------------------------------------------------------------+
//| Fungsi untuk cek TP2 pada posisi SELL                           |
//+------------------------------------------------------------------+
void CheckTP2_Sell(int total_sell)
{
   int profit_count = 0;
   double current_ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
   
   // Hitung berapa posisi SELL yang sedang profit
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ulong ticket = PositionGetTicket(i);
      if(ticket > 0)
      {
         ...
         {
            double open_price = PositionGetDouble(POSITION_PRICE_OPEN);
            // SELL profit jika current_ask < open_price
            if(current_ask < open_price)
            {
               profit_count++;
            }
         }
   ...
   // Hitung persentase profit
   double profit_percentage = (double)profit_count / (double)total_sell * 100.0;
   
   // Jika persentase profit >= threshold, close all SELL
   if(profit_percentage >= InpTP2Percentage)
   {
      Print("TP2 SELL TRIGGERED! Profit: ", profit_count, "/", total_sell, " (", 
            DoubleToString(profit_percentage, 2), "%) - Closing all SELL positions...");
...
//+------------------------------------------------------------------+
//| Fungsi untuk close semua posisi BUY                             |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Fungsi untuk close semua posisi SELL                            |
//+------------------------------------------------------------------+
...
void CheckAndOpenSell(double current_rsi, int buy_count, int sell_count)
{
   ...
   if(sell_count == 0)
   {
      ...
      if(rsi_cross_ob || current_rsi >= InpRSI_OB)
      {
         ...
         // TP hanya diset jika posisi <= threshold TP2
         if(sell_count < InpTP2MinPositions)
            tp_price = entry_price - (InpTPPoints * _Point);
